      ...
      <div className="hidden lg:flex flex-row items-center justify-center gap-10 px-8 py-3 bg-black/95 backdrop-blur text-white border-b-2 border-white">
        {links.map((link) => (
          <Link
            href={link.href}
            key={link.label}
            className={`relative py-1 text-lg uppercase tracking-wide transition-colors after:absolute after:left-0 after:-bottom-1 after:h-0.5 after:bg-pink-400 after:transition-all after:duration-300 ${
              link.active
          ...
          <div className="flex flex-col divide-y divide-white/10 border-t-2 border-white">
            {links.map((link) => (
              <Link
                href={link.href}
                key={link.label}
                className={`p-4 pl-6 text-xl uppercase border-l-4 transition-colors ${
                  link.active
